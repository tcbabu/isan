...
case 1 :
                 ...
                 break;
case 2 : 
                 _box_fill(x-3*fac,y-3*fac,x+3*fac,y+3*fac,(unsigned int)c_color);
                 break;
...
break;
        case 9 :
                 ...
                 _poly_fill((short)3,px,py,(short)0,(unsigned int)c_color);
                 break;
        case 10:
                 px[0]=x-4*fac;py[0]=y;px[1]=x+4*fac;py[1]=y+4*fac;
                 px[2]=x+4*fac;py[2]=y-4*fac;
...
int berzier_o(int *xp,int *yp,float x1,float y1,float x4,float y4,float x2,
...
void _uimpoly_fill(kgWC *wc,int n,int *px,int *py,int *px1,int *py1,unsigned int b_color)
{
 ...
 /*displaying scan lines*/
      for (i=0;i<cnt;i+=2) {
         _uimhori_line(wc,(int)(xcord[i]/Mag+0.5),(int)(xcord[i+1]/Mag),
               ((float)scan/Mag+0.5));
      }
      scan++;
   }
     ...
     while ( code >= 0)
      {
        ...
        pnts +=ui_fillchr(wc,x,y,n,&x1[pnts],&y1[pnts],&x2[pnts],&y2[pnts]);
        code= dc->icxv[ptr];n=dc->icyv[ptr];
        if(code < 0)
         {
          _uimpoly_fill(wc,pnts,x1,y1,x2,y2,color);
          temp =wc->c_color;wcset_clr(wc,dc->t_color);
          for(i=0;i<pnts;i++) uidraw_line(G,x1[i]/Mag,y1[i]/Mag,x2[i]/Mag,y2[i]/Mag);
          wcset_clr(wc,temp);
          pnts=0;
         }
      }
...
void _uipoly_fill(DIG *G,int n,int *px,int *py,int flag,unsigned int color)
{
 ...
 /*displaying scan lines*/
    for (i=0;i<cnt;i+=2)
      _uihori_line(wc,xcord[i],xcord[i+1],scan);
      scan++;
  }
  
  wcset_clr(wc,dc->ln_color);
  if (flag==1)
      for(i=0;i<n;i++) _uidraw_line(wc,px[i],py[i],px[(i+1)%n],py[(i+1)%n]);
  wcset_clr(wc,tempc);
}
...
pmin[j+1]=tpn;
 ...
 /*displaying scan lines*/
   for (i=0;i<cnt;i+=2) uigodr_line(G,xcord[i],xcord[i+1],scan,pcord[i],pcord[i+1]);
   scan++;
  }
  if (flag==1)
      for(i=0;i<n;i++) _uidraw_line(wc,px[i],py[i],px[(i+1)%n],py[(i+1)%n]);
...
// uiset_sup_clip_limits(wc,(dc->v_x1+dc->D_x),D->evgay-(dc->v_y2+dc->D_y),(dc->v_x2+dc->D_x),D->evgay-(dc->v_y1+dc->D_y));
// uiset_clip_limits(wc,(dc->v_x1+dc->D_x),D->evgay-(dc->v_y2+dc->D_y),(dc->v_x2+dc->D_x),D->evgay-(dc->v_y1+dc->D_y));
   uiset_sup_clip_limits(wc,(dc->D_x),D->evgay-(dc->V_y+dc->D_y),(dc->V_x+dc->D_x),D->evgay-(dc->D_y));
   uiset_clip_limits(wc,(dc->D_x),D->evgay-(dc->V_y+dc->D_y),(dc->V_x+dc->D_x),D->evgay-(dc->D_y));
...
void ui_window(DIG *G,float x1,float y1,float x2,float y2) {
  ...
  if((G->D != NULL) )ui_vu_port(G,dc->vu_x1,dc->vu_y1,dc->vu_x2,dc->vu_y2);
  if(G->D_ON==0)img_viewport(G,dc->vu_x1,dc->vu_y1,dc->vu_x2,dc->vu_y2);
}
...
//  uiWriteImage(rzimg,"Junk.png");
 ...
 void ui_drawimage(DIG *G,void *imgfile,float x1,float y1,float x2,float y2)
  ...
  w = X2-X1+1;
  h = Y2 -Y1+1;
  kgImage(D,img,X1,Y1,img->image_width,img->image_height,0.0,1.0);
  if(!IMG) uiFreeImage(img);
  }
...
void ui_txt_size(DIG *G,float ht,float wt,float sp,float htx,float wty,float spy) {
  ...
  dc->txt_wty=wty;
  dc->txt_spy=spy;
  dc->txt_wt= dc->sint*dc->sint*dc->txt_wty*(dc->w_x2-dc->w_x1)/(dc->w_y2-dc->w_y1)+dc->cost*dc->cost*dc->txt_wtx;
  dc->txt_sp= dc->sint*dc->sint*dc->txt_spy*(dc->w_x2-dc->w_x1)/(dc->w_y2-dc->w_y1)+dc->cost*dc->cost*dc->txt_spx;
  dc->txt_ht= dc->sint*dc->sint*dc->txt_htx/(dc->w_x2-dc->w_x1)*(dc->w_y2-dc->w_y1)+dc->cost*dc->cost*dc->txt_hty;
...
/*    txt_bold=bold;*/
      ...
      while (pt!=NULL){
       ...
       if(pt->p =='U') {
        _uimove(G,uiTX((pt->x1),pt->ymin),uiTY((pt->x1),pt->ymin));
        _uidraw(G,uiTX((pt->x2),pt->ymin),uiTY((pt->x2),pt->ymin));
       }
       else{
        _uimove(G,uiTX((pt->x1),pt->ymax),uiTY((pt->x1),pt->ymax));
        _uidraw(G,uiTX((pt->x2),pt->ymax),uiTY((pt->x2),pt->ymax));
       }
       dc->O_L=pt;
       pt=pt->Nx;
       free(dc->O_L);
  ...
  void ui_clr_vu(DIG *G) {
    ...
    if(G->D_ON ==0 ) {
       ...
       _uibox_fill(G->wc,dc->D_x+dc->v_x1,dc->D_y+dc->v_y1,dc->v_x2+dc->D_x,dc->v_y2+dc->D_y,0);
...
void ui_panel(DIG *G,float *x,float *y,int color,int flag,int n)
  ...
  if( (j>2)&&(j<500)) _uipoly_fill(G,j,x1,y1,flag,(unsigned int)color);
  free(x1),free(y1);
}
...
void  ui_clip_limit(DIG *G,float x1,float y1,float x2,float y2)
  {
    ...
    if(iy1>=iy2) {
     ...
     if( ix2<wc->c_v_x1) {wc->c_v_x1=1,wc->c_v_x2=0,wc->c_v_y1=1,wc->c_v_y2=0;return;};
     if( ix1>wc->c_v_x2) {wc->c_v_x1=1,wc->c_v_x2=0,wc->c_v_y1=1,wc->c_v_y2=0;return;};
     if( iy2<wc->c_v_y1) {wc->c_v_x1=1,wc->c_v_x2=0,wc->c_v_y1=1,wc->c_v_y2=0;return;};
     if( iy1>wc->c_v_y2) {wc->c_v_x1=1,wc->c_v_x2=0,wc->c_v_y1=1,wc->c_v_y2=0;return;};
...
void ui_PointerZoomPan(DIG *G,float *xmin,float *ymin, float *xmax,float *ymax){
      ...
      while(1) {
        ...
        if(key <= 0) continue;
        if((xpo <dc->v_x1)||(xpo >dc->v_x2) ||((EVGAY -ypo) < dc->v_y1) ||((EVGAY -ypo) >dc->v_y2)) break;
        switch(key) {
           case 1:
	     dx *= 0.95;
	     dy *= 0.95;
...
void pick_box(DIG *G,float *x1,float *y1,float *x2,float *y2)
{
      ...
      short it=6,item=1;
      char *cmenu[] ={
            "Diagonal",   "move_Around","Stretch","Quit","menuOff","Usepointer",NULL};
      DIALOG *D;
      kgWC *wc;
      wc = G->wc;
      D = G->D;
...
//TCB
  ...
  while(kgCheckEvent(D,&kbo)==0) { count++; usleep(10000);if(count>100) return;}
  usleep(10000);
  count=0;
...
//       if(isdigit(kb)) {jmp = kb-'0';continue;}
       ...
       if(ui_Uparrow(kb)) { cursor_up(dc);draw_box_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Downarrow(kb)) { cursor_down(dc);draw_box_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Leftarrow(kb)) { cursor_left(dc);draw_box_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Rightarrow(kb)) { cursor_right(dc);draw_box_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       break;
     default:
       break;
    }
...
//       if(isdigit(kb)) {jmp = kb-'0';continue;}
       ...
       if(ui_Uparrow(kb)) { cursor_up(dc);draw_rect_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Downarrow(kb)) { cursor_down(dc);draw_rect_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Leftarrow(kb)) { cursor_left(dc);draw_rect_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Rightarrow(kb)) { cursor_right(dc);draw_rect_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       break;
     default:
       break;
    }
...
//       if(isdigit(kb)) {jmp = kb-'0';continue;}
       ...
       if(ui_Uparrow(kb)) { cursor_up(dc);draw_cross_cursor(G,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Downarrow(kb)) { cursor_down(dc);draw_cross_cursor(G,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Leftarrow(kb)) { cursor_left(dc);draw_cross_cursor(G,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Rightarrow(kb)) { cursor_right(dc);draw_cross_cursor(G,dc->gcur_x,dc->gcur_y);break; }
       break;
     default:
       break;
    }
...
char cross_getpointer(DIG *G,int *xx,int *yy)
  ...
  for(;;)
  {
 while( (pointer=cross_pointer(D,&xpo,&ypo,&entry))<0)
    CALL_ATTN;
  while((pointer==0))
    {
...
//       if(isdigit(kb)) {jmp = kb-'0';continue;}
       ...
       if(ui_Uparrow(kb)) { cursor_up(dc);draw_rbr_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Downarrow(kb)) { cursor_down(dc);draw_rbr_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Leftarrow(kb)) { cursor_left(dc);draw_rbr_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       if(ui_Rightarrow(kb)) { cursor_right(dc);draw_rbr_cursor(G,xorg,yorg,dc->gcur_x,dc->gcur_y);break; }
       break;
     default:
       break;
    }
...
//End
  ...
  if(( x_min<=(D->evgax+1)) && (x_max1>=0)) {
    ...
    if(	(y_min<=(D->evgay)) && (y_max>=0)) {
      ...
      if(y_max>(D->evgay)) y_max = (D->evgay);
      if(y_min<0) y_min = 0;
      _dvBox_Fill_Transparent(wc,x_min,(D->evgay)-y_max,x_max1,(D->evgay)-y_min,transparency);
    }
  }
  uiset_clr(D,temp);
  return;
...
//End
  ...
  if(( x_min<=(D->evgax)) && (x_max1>=0)) {
    ...
    if(( y_min<=(D->evgay)) && (y_max>=0)) {
      ...
      if(y_max>(D->evgay)) y_max = (D->evgay);
      if(y_min<0) y_min = 0;
      _dvBox_Fill_Transparent(wc,x_min,y_min,x_max1,y_max,transparency);
    }
  }
  uiset_clr(D,temp);
  return;
...
//   if(width>height) width=height;
   ...
   if(fid != NULL ) {
      ...
      if(status >=0) {
        ...
        kgRoundedRectangleFill(fid,xp+(float)BxSize,yp+0.25*BxSize,(float)BxSize,(float)BxSize,0,15,0.5);
        kgRoundedRectanglePressed(fid,xp+(float)BxSize,yp+0.25*BxSize,(float)BxSize,(float)BxSize,color,0.5,4);
        kgMove2f(fid,xp+(float) BxSize+0.5*FontSize,yp);
        if(status > 0) {
          kgArcFill(fid,xp+BxSize,yp+0.25*BxSize,(float)BxSize*0.2,0.,360.,0,0);
        }
      }
      img=kgGetResizedImage(fid);
      kgCloseImage(fid);
...
//   if(width>height) width=height;
   ...
   if(fid != NULL ) {
      ...
      if(status >=0) {
        ...
        kgCircle2f(fid,1.0*BxSize,yp+0.25*BxSize,(float)(BxSize+2)*0.35);
        kgCircle2f(fid,1.0*BxSize,yp+0.25*BxSize,(float)(BxSize+3)*0.35);
#else
        kgGetDefaultRGB(color,&r,&g,&b);
        rf =r,gf=g,bf=b;
        kgRoundedRectangleRing(fid,1.0*BxSize,yp+0.25*BxSize,(float)BxSize*0.7,(float)BxSize*0.7,rf,gf,bf,0.5,0.7);
#endif
...
//   if(Str[0]=='\0') return;
   ...
   if(fid != NULL ) {
      ...
      if(status >=0) {
        ...
        kgCircle2f(fid,1.0*BxSize,xp+0.25*BxSize,(float)(BxSize+2)*0.35);
        kgCircle2f(fid,1.0*BxSize,xp+0.25*BxSize,(float)(BxSize+3)*0.35);
#endif
...
//        length = uiStringLength(Str,(int)(tw*fac));
      ...
      if(status >=0) {
        ...
        kgChangeColor(fid,0,0,0,0);
        kgChangeColor(fid,15,255,255,255);
        kgRoundedRectangleFill(fid,(float)1.0*BxSize,xp+0.25*BxSize,(float)BxSize,(float)BxSize,0,15,0.5);
        kgRoundedRectanglePressed(fid,(float)1.0*BxSize,xp+0.25*BxSize,(float)BxSize,(float)BxSize,D->gc.fill_clr,0.5,4);
#endif
        kgMove2f(fid,(float) BxSize+0.5*FontSize,xp);
        if(status > 0) {
          kgArcFill(fid,1.0*BxSize,xp+0.25*BxSize,(float)BxSize*0.2,0.,360.,0,0);
        }
      }
//      img=kgGetResizedImage(fid);
      img=kgGetSharpImage(fid);
...
//   if(str==NULL) return;
//   if(str[0]=='\0') return;
   img = kgBoxedStringToImage(str,NULL,ln,height,font,border,highli,charclr,justfic,FontSize,rfac,state);
   if(img != NULL) {
     kgImage(D,img,x,y,ln,height,0.0,1.0);
     uiFreeImage(img);
   }
...
//   if(str==NULL) return;
//   if(str[0]=='\0') return;
   img = kgFilledStringToImage3(str,NULL,ln,height,font,fillcolor,highli,color,D->gc.fill_clr,justfic,FontSize,rfac,state,depthfac);
   if(img != NULL) {
     kgImage(D,img,x,y,ln,height,0.0,1.0);
     uiFreeImage(img);
   }
...
//   if(str==NULL) return;
//   if(str[0]=='\0') return;
   img = kgFilledStringToImage1(str,NULL,ln,height,font,fillcolor,highli,color,D->gc.fill_clr,justfic,FontSize,rfac,state,depthfac);
   if(img != NULL) {
     kgImage(D,img,x,y,ln,height,0.0,1.0);
     uiFreeImage(img);
   }
...
//   if(str==NULL) return;
//   if(str[0]=='\0') return;
   img = kgSplashStringToImage(str,ln,height,font,fillcolor,color,D->gc.fill_clr,justfic,FontSize,rfac);
   if(img != NULL) {
     kgImage(D,img,x,y,ln,height,0.0,1.0);
     uiFreeImage(img);
   }
...
*/
   ...
   switch(type) {
     case 0:
       uiString(D,str,x,y+height+5,ln,20,font,color,FontSize,justfic,fillcolor);
       return;
     case 1:
       uiBoxedString(D,str,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state);
       return;
     case 2:
       uiFilledString(D,str,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state,3.0);
       return;
     case 3:
       uiFilledString1(D,str,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state,5.0);
       return;
     case 4:
     case 5:
     case 6:
       img = kgShadedStringToImage(str,NULL,ln,height,font,fillcolor,highli,color,justfic,FontSize,rfac,state,type-4);
       if(img != NULL) {
         kgImage(D,img,x,y,ln,height,0.0,1.0);
         uiFreeImage(img);
       }
       return;
     default:
       uiFilledString(D,str,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state,(float)(type-6));
       return;
   }
}
...
*/
   ...
   if((image==NULL)&&(type!=0)) {
     uiShadedString(D,str,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state,type);
     return;
   }
#endif
   wc = D->wc;
   ...
   switch(type) {
     case 0:
       ...
       return;
     case 1:
       uiBoxedString(D,NULL,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state);
       if(image != NULL) { kgImage(D,image,x+1,y+1,width-2,height-2,0.0,1.0); }
       if((str!=NULL)&&(str[0]!='\0')) uiString(D,str,x+off,y,ln,height,font,color,FontSize,justfic,-1);
       return;
     case 2:
       uiFilledString(D,NULL,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state,3.0);
       if(image != NULL) { kgImage(D,image,x+1,y+1,width-2,height-2,0.0,1.0); }
       if((str!=NULL)&&(str[0]!='\0')) uiString(D,str,x+off,y,ln,height,font,color,FontSize,justfic,-1);
       return;
     case 3:
       uiFilledString1(D,NULL,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state,3.0);
       if(image != NULL) { kgImage(D,image,x+1,y+1,width-2,height-2,0.0,1.0); }
       if((str!=NULL)&&(str[0]!='\0')) uiString(D,str,x+off,y,ln,height,font,color,FontSize,justfic,-1);
       return;
     case 4:
     ...
     case 6:
       ...
       return;
     default:
       uiFilledString1(D,NULL,x,y,width,height,font,fillcolor,highli,color,FontSize,justfic,rfac,state,(float)(type-3.));
       if(image != NULL) { kgImage(D,image,x+1,y+1,width-2,height-2,0.0,1.0); }
       if((str!=NULL)&&(str[0]!='\0')) uiString(D,str,x+off,y,ln,height,font,color,FontSize,justfic,-1);
       return;
   }
...
//       uiString(D,str,x,y+height+5,width,20,font,color,FontSize,justfic,fillcolor);
       ...
       if((gimg->image_width > width)||(gimg->image_height > height)) {
         ...
         if((str!=NULL)&&(str[0]!='\0')){
           img = kgStringToImage(str,NULL,ln,height,font,color,justfic,FontSize,-1);
           if(img != NULL) {
     ...
     case 1:
       if(cimg != NULL) {
         ...
         img = kgBoxedStringToImage(str,NULL,width,height,font,fillcolor,
              highli,color,justfic,FontSize,rfac,state);
       }
       return img;
     case 2:
...
//              img = kgMergeImages(img,cimg,off,0);
     ...
     case 3:
       if(cimg != NULL) {
          ...
          img1=kgFilledStringToImage3(NULL,NULL,width-2,height-2,font,fillcolor,
                 highli,color,D->gc.fill_clr,justfic,FontSize,rfac,state,3.0);
          img = kgMergeImages(img,img1,0,0);
          img = kgMergeImages(img,cimg,poff,poff);
          kgFreeImage(cimg);
          if((str!=NULL)&&(str[0]!='\0')){
             cimg = kgStringToImage(str,NULL,ln,height,font,color,justfic,
                                     FontSize,-1);
             if(cimg != NULL) {
               img = kgMergeImages(img,cimg,off,0);
               kgFreeImage(cimg);
          ...
          img = kgFilledStringToImage3(NULL,NULL,width,height,font,fillcolor,
                 highli,color,D->gc.fill_clr,justfic,FontSize,rfac,state,2.0);
          img1 =kgFilledStringToImage3(str,NULL,width-2,height-2,font,fillcolor,
                 highli,color,D->gc.fill_clr,justfic,FontSize,rfac,state,3.0);
          img = kgMergeImages(img,img1,0,0);
       }
       return img;
     ...
     case 8:
       if(cimg != NULL) {
         ...
         img = kgShadedStringToImage(str,NULL,width,height,font,fillcolor,
                 highli,color,justfic,FontSize,rfac,state,type-4);
       }
       return img;
     default:
...
void * kgSetupGraphics(void *Tmp) {
  ...
  DIALOG *D;
  D = (DIALOG *)Tmp;
  if((D->parent!= NULL)&&(D->SUBWIN==1)) wc=kgCreateSubWindow(Tmp);
...
void _uiborder3d(void *parent,int xmin,int ymin,
            int xmax,int ymax){
 int temp;
 DIALOG *D;
 D= (DIALOG *)(parent);
...
void _uibordertype1(DIALOG *D,int xmin,int ymin,
            int xmax,int ymax){
 int temp;
 temp = WC(D)->c_color;
...
void _uibordertype2(DIALOG *D,int xmin,int ymin,
            int xmax,int ymax,
            int tw,int lw,int rw,int bw){
 int temp;
 temp = WC(D)->c_color;
 uiset_clr(D,D->gc.vbright);
 _ui_h_line(WC(D),xmin,xmax,ymin);
...
void _uibordertype3(DIALOG *D,int xmin,int ymin,
            int xmax,int ymax,
            int tw,int lw,int rw,int bw){
 int temp;
 temp = WC(D)->c_color;
...
void _uibordertype4(DIALOG *D,int xmin,int ymin,
            int xmax,int ymax) {
 int temp;
 temp = WC(D)->c_color;
 uiset_clr(D,D->gc.bright);
...
void * make_but_str(DIALOG *D,int x1,int y1,int xgap,int ygap,int nxb,int nyb,
...
// Should not be done
//   if(color >= 0) _uibox_fill(wc,x1+2,EVGAY-y1+2,x2-2,EVGAY-y2-2,(unsigned int) color);
   if( buf != NULL) kgImage(D,bt->xpm[k],x0+1,y0+1,l-2,w-2,transparency,highfac);
   uiRest_clip_limits(wc);
}
...
void _uiHighButton(BUTS *bt,int df) {
   int xgap=0,ygap=0,i,j,k,xx,yy,width,height,xd,yd,Hsize;
   int *sw;
   DIALOG *D;
   kgWC *wc;
   D= (DIALOG *)(bt->D);
...
//   Get_resolution(&EVGAX,&EVGAY);
   if((bt->nyb)>1)ygap = ((float)((bt->y2-bt->y1)-height*bt->nyb)/(bt->nyb-1.0) +0.5);
   if((bt->nxb)>1)xgap = (float)((bt->x2-bt->x1)-width*bt->nxb)/(bt->nxb-1.0) +0.5;
   sw = bt->sw;
   k=0;
   if(df != bt->df){
    for (j=0;j<(bt->nyb);j++){
      for(i=0; i<(bt->nxb);i++){
        if(k+1 == bt->df) {
          ...
          xx=bt->x1+ i*(bt->b_w+xgap);
           if(sw[k]==1) uirmv_inset_protru(D,xx-Hsize,yy+height+Hsize,xx+width+Hsize,yy-Hsize);
        }
        if(k+1 == df){
          yy=bt->y2- j*(bt->b_h+ygap)-bt->b_h;
          xx=bt->x1+ i*(bt->b_w+xgap);
          if(sw[k]==1) uidraw_depre(D,xx-Hsize,yy+height+Hsize,xx+width+Hsize,yy-Hsize);
        }
        k++;
      }
    }
...
void _uiHighNewButton(BUTS *bt,int df) {
   ...
   width=bt->b_w;height=bt->b_h;
   if((bt->nyb)>1)
       ygap =((float)((bt->y2-bt->y1)-height*bt->nyb)/(bt->nyb-1.0) +0.5);
   if((bt->nxb)>1)
       xgap = (float)((bt->x2-bt->x1)-width*bt->nxb)/(bt->nxb-1.0) +0.5;
   k=0;
   for (j=0;j<(bt->nyb);j++){
     yy=bt->y2- j*(bt->b_h+ygap)-bt->b_h;
     for(i=0; i<(bt->nxb);i++){
       xx=bt->x1+ i*(bt->b_w+xgap);
       _ui_draw_newbutton(xx,yy+height,xx+width,yy,bt,k,D->transparency);
#if 0
       if((bt->sw[k] ==1)||(bt->sw[k]==3) ) {
          if(bt->D->butattn ) {
           if(k+1 == df) {
             uidraw_depre(D,xx-bt->Hsize,yy+height+bt->Hsize,xx+width+bt->Hsize,yy-bt->Hsize);
           }
           else {
             uirmv_inset_protru(D,xx-bt->Hsize,yy+height+bt->Hsize,xx+width+bt->Hsize,yy-bt->Hsize);
           }
          }
       }
#endif
 ...
 int uiCheckClickPosition( int x1,int y1,int x2,int y2,int PON_X,int PON_Y) {
    ...
    xp = PON_X;
    yp = PON_Y;
    if((x1>(xp))||(x2<(xp))||(y2<(yp))||(y1>(yp))) return(0);
    else return(1);
 }
...
//   if(bt->sw[k]!=  1) return; //not active
   ...
   if(bt->xpm != NULL) {
       ...
       if((bt->xpm != NULL)){
           if(bt->xpm[k] != NULL)_ui_jpg_draw(bt,k,x1,D->evgay-y2+1,x2,D->evgay-y1+1,bkgr);
...
//     uiMenustr(D,x1+15,y2+8,bt->title[k],D->gc.but_char);
     uiString(D,bt->title[k],x1+4,D->evgay-(y2+24),(x2-x1-8),22,D->gc.ButtonFont,D->gc.but_char,D->gc.FontSize,0,-1);
     uiset_clr(D,D->gc.v_dim);
...
void _ui_draw_pressed_newbutton(int x1,int y1,int x2,int y2,BUTS *bt,int k,float transparency) 
 {
   ...
   uiBkup_clip_limits(wc);
   uiSet_full_scrn(wc);
   _uirect_fill_transparent(wc,x1-xgap,y1+ygap,x2+xgap,y2-ygap,D->gc.fill_clr,transparency);
   if(bt->bkgr == NULL) bkgr = D->gc.fill_clr;
   else bkgr = bt->bkgr[k];
   if(bkgr < 0) {
     if(bkgr==-1) bkgr=D->gc.fill_clr;
   }
   uiShadedStringImage(D,bt->title[k],xpm,x0,(y0),l,w,D->gc.ButtonFont,bkgr,
                              highli,D->gc.but_char,D->gc.FontSize,0,bt->fac,-1,bt->type);
   uiset_clr(D,temp);
   uiRest_clip_limits(wc);
 }
...
//     printf("Inside _ui_draw_button:title:%s\n",bt->title[k]);
   ...
   if(bt->xpm != NULL) {
       ...
       if((bt->xpm != NULL)&&(bt->xpm[k] != NULL)) _ui_jpg_draw(bt,k,x1,D->evgay-y2+1,x2,D->evgay-y1+1,bkgr);
       else if(bkgr>=0) _uirect_fill(wc,x1+4,y1-4,x2-4,y2+4,bkgr);
   }
   if(bt->sw[k]==  1) {
//     uiMenustr(D,x1+15,y2+8,bt->title[k],D->gc.but_char);
...
void _dv_draw_newbutton(void *tmp,int k,int state) { 
   ...
   switch (B->code ) {
     case 'n':
       ...
       if(D->DrawBkgr!= 0)_dvrect_fill_transparent(wc,x1,y1,x2,y2,
                 D->gc.fill_clr,transparency);
//       switch(butn[k].state) {
       switch(state) {
         case 0:
           img = butn[k].imgn;
     ...
     default:
       ...
       uiBkup_clip_limits(wc);
       uiSet_full_scrn(wc);
       if(D->DrawBkgr!= 0)_dvrect_fill_transparent(wc,x1,y1,x1+ln+xgap,y1+wd+ygap,
                 D->gc.fill_clr,transparency);
      ...
      uiShadedStringImage(D,B->titles[k],img,x1+xgap/2,y1+ygap/2,ln,wd,D->gc.ButtonFont,bkgr,
           highli,D->gc.but_char,D->gc.FontSize,0,B->fac,state,B->type);
      uiset_clr(D,temp);
      uiRest_clip_limits(wc);
     break;
   }
...
//     _dvrect_fill_transparent(wc,x1,y1,x2,y2,D->gc.fill_clr,transparency);
   ...
   if(bkgrdraw) {
     ...
     if(butn[k].Bimg== NULL) butn[k].Bimg= kgGetBackground(D,x1,y1,x2,y2);
//     else kgImage(D,butn[k].Bimg,x1,y1,(x2-x1+1),(y2-y1+1),0.0,1.0);
     else kgRestoreImage(D,butn[k].Bimg,x1,y1,(x2-x1+1),(y2-y1+1));
   }
//       switch(butn[k].state) {
   switch(state) {
         case 0:
...
void _ui_draw_newbutton(int x1,int y1,int x2,int y2,BUTS *bt,int k,float transparency) 
 {
   ...
   uiShadedStringImage(D,bt->title[k],xpm,x0,y0,l,w,D->gc.ButtonFont,bkgr,
                              highli,D->gc.but_char,D->gc.FontSize,0,bt->fac,0,bt->type);
   uiset_clr(D,temp);
   uiRest_clip_limits(wc);
 }
...
//   _uirect_fill_transparent(wc,x1+1,y1-1,x2-1,y2+1,D->gc.fill_clr,transparency);
   ...
   highli=0;
   uiShadedString(D,bt->title[k],x1,D->evgay-(y1),(x2-x1),(y1-y2),D->gc.ButtonFont,bkgr,
                              highli,D->gc.but_char,D->gc.FontSize,0,bt->fac,0,bt->type);
   if((bt->xpm != NULL)&&(bt->xpm[k] != NULL)) {
         _ui_jpg_newdraw(bt,k,x1,EVGAY-y2,x2,EVGAY-y1,bkgr,0.0,1.0);
   }
   uiset_clr(D,temp);
   uiRest_clip_limits(wc);
 }
   ...
   for (j=0;j<(bt->nyb);j++){
     ...
     for(i=0; i<(bt->nxb);i++){
       ...
       if((bt->sw[k] ==1)||(bt->sw[k]==3) ) {
          if(bt->D->butattn ) {
           if(k+1 == bt->df) {
             uidraw_depre((bt->D),xx-bt->Hsize,yy+height+bt->Hsize,xx+width+bt->Hsize,yy-bt->Hsize);
           }
           else {
             _ui_rmv_inset_protru((bt->D),xx-bt->Hsize,yy+height+bt->Hsize,xx+width+bt->Hsize,yy-bt->Hsize);
           }
          }
       }
       k++;
   ...
   for (j=0;j<(bt->nyb);j++){
     for(i=0; i<(bt->nxb);i++){
       if(k==kk) {
         yy=bt->y2- j*(bt->b_h+ygap)-bt->b_h;
         xx=bt->x1+ i*(bt->b_w+xgap);
         _ui_draw_pressed_newbutton(xx,yy+height,xx+width,yy,bt,k,bt->D->transparency);
       }
       kk++;
     }
   }
...
void _uiDrawNewButns(BUTS *bt) {
   ...
   for (j=0;j<(bt->nyb);j++){
     ...
     for(i=0; i<(bt->nxb);i++){
       xx=bt->x1+ i*(bt->b_w+xgap);
       _ui_draw_newbutton(xx,yy+height,xx+width,yy,bt,k,bt->D->transparency);
#if 0
#endif
       k++;
     }
...
//   uidia_message(D,b->x1+x1,b->y1+12+y1+b->width/2,b->prompt+i);
   ...
   b->buts = make_but_str(D,b->x1+x1+lngth,b->y1+y1+Hsize+2,b->xgap,b->ygap,b->nx,b->ny,
        b->lngth,b->width,*(b->df),b->titles,b->xpm,b->bkgr,NULL);
   buts= (BUTS *)(b->buts);
   buts->D = D;
   buts->Hsize = D->Hsize;
...
int _uiMake_B(DIBN *b) {
  ...
  if(b->hide != 1){
    ...
    uiSet_full_scrn(wc);
    if(b->bordr==1) {
      _dv_draw_bound(D,(x1+2),(y1+2),(x2-2), (y2-2),D->gc.high_clr);
    }
    _uiDrawButtons(b);
    uiRest_clip_limits(wc);
  }
...
int _uiMake_N(void *tmp) {
  ...
  if(buts[0].imgn==NULL) {
     n = b->nx*b->ny;
     for(k=0;k<n;k++) {
       if(buts[k].imgn!= NULL) {kgFreeImage(buts[k].imgn); buts[k].imgn=NULL;}
       if(buts[k].imgp!= NULL) {kgFreeImage(buts[k].imgp); buts[k].imgp=NULL;}
       if(buts[k].imgh!= NULL) {kgFreeImage(buts[k].imgh); buts[k].imgh=NULL;}
...
//    int offset=4;
    ...
    if(D->DrawBkgr!= 0) {
      _dvbordertype0(D, x1-(offset),y1-(offset),x2+(offset),
                      y2+(offset),D->gc.vbright);
    }
    uiRest_clip_limits(wc);
    return 1;
}
...
void _ui_slidevalue(DIALOG *D,S_STR *pt) {
   ...
   x = pt->x;
   y = pt->y;
   _dvrect_fill_transparent(wc,x,y-w,x+l,y-1,D->gc.fill_clr,D->transparency);
   y = EVGAY-y;
...
//   _dvrect_fill_transparent(wc,x,y-w-3,x+l,y+w+3,gc.fill_clr,D->transparency);
   kgRestoreImagePart(D,Bimg,x,y-w-3,(x-x1),-(y1-y+w+3),l+1,w+w+7);
   y = EVGAY-y;
//   _uirect_fill(wc,x,y+w,x+l,y-w,gc.fill_clr);
//   _uirect_fill(wc,x+w+1+sw/2,y-pt->Sdpw,x+l-w-1-sw/2,y+pt->Sdpw-w,gc.fill_clr);
//   uidraw_depre(D,x+w+1,y-Sdpw,x+l-w-1,y+Sdpw-w);
...
//   _rect_fill(x,y+w,x+l,y-w,pt->fill_clr);
   switch(f->type) {
    default:
     y = EVGAY-y;
     _uirect_fill_transparent(wc,x+w+1+sw/2,y-pt->Sdpw,x+l-w-1-sw/2,y+pt->Sdpw-w,pt->fill_clr,D->transparency);
...
void _ui_movehslidepointer(DIHB *B) {
   ...
   y2 = B->y2-offset+D->yo;
#ifndef D_RESTORE
   _dvrect_fill_transparent(wc,x1,y1,x2,y2,gc->fill_clr,D->transparency);
#else
...
//TCBTCB
#ifndef D_RESTORE
   _uirect_fill_transparent(wc,x1-2,y1+5,x2+2,y2-5,gc->fill_clr,D->transparency);
   _uirect_fill(wc,x1+1,y1-1,x2-2,y2+2,gc->fill_clr);
#else
   kgRestoreImagePart(D,Bimg,x1-2,EVGAY-y1-5,(x1-2-xo),(EVGAY-y1-4-yo),(x2+5-x1),abs(y2-9-y1));
   kgRestoreImagePart(D,Bimg,x1+1,EVGAY-y1+1,(x1+1-xo),(EVGAY-y1+2-yo),(x2-2-x1),abs(y2+4-y1));
#endif
...
void * _ui_setslide(void *tmp,int min,int max,int x,int y,int l,int df,
...
//        uiString(D,f->prompt+i,f->x1+D->xo+2,f->y1+D->yo+21,n*9-5,20,D->gc.PromptFont,D->gc.txt_pchar,D->gc.FontSize,1,D->gc.fill_clr);
        uiString(D,f->prompt+i,f->x1+D->xo+2,f->y1+D->yo+21,n*9-5,20,D->gc.PromptFont,D->gc.txt_pchar,D->gc.FontSize,1,-1);
     }
   }
  _ui_slidebar(D,ptr);
  ...
  _ui_slidevalue(D,ptr);
    ...
    uiRest_clip_limits(wc);
#endif
    if(f->Bimg != NULL) kgRestoreImage(D,f->Bimg,x1,y1,(x2-x1+1),(y2-y1+1));
 }
 return ret;
}
...
void * _ui_sethbar(DIHB *B,int min,int max,int x,int y,int l,int df,
      char ch,double cf,double sh) 
 {
   DIALOG *D;
   S_STR *ptr;
...
int Make_f_bar(DIF *f) {
   ...
   _ui_setslide(f,min,max,(int)f->x1+lngth+x1+2,
            (int)f->y1+y1+22, (int)f->lngth,df,'f',ch,sh);
   return(ret);
 }
...
static void _uiMakeSBarImages_o(DIHB *B) {
 ...
 kgRoundedRectangleFill(fid,(float)-1.0,0.0,(float)(l-of),(float)(w-of),0,D->gc.v_dim,0.5);
 kgRoundedRectangleFill(fid,(float)0.0,0.0,(float)(l-of),(float)(w-of),0,D->gc.dim,0.5);
...
// kgRoundedRectangleRaised(fid,0.,0.,(float)(l-of),(float)(w-of),fillcolor,0.5,1);
 ...
 vmin= vmax*0.75;
#if 0
 kgRoundedRectangleFill(fid,-1.0,0.0,
                          (float)xsize,(float)ysize,0,15,rfac);
 kgRoundedRectangleFill(fid,1.0,-1.0,
                          (float)xsize,(float)ysize,0,0,rfac);
#endif
 switch(type) {
        case 0:
         kgRoundedRectangleShade0(fid,0.0,0.0,(float)xsize,(float)ysize,fillcolor,rfac,vmin,vmax);
        break;
        case 1:
         kgRoundedRectangleShade1(fid,0.0,0.0,(float)xsize,(float)ysize,fillcolor,rfac,vmin,vmax*0.95);
        break;
        case 2:
         kgRoundedRectangleFill(fid,-1.0,1.0,(float)xsize,(float)ysize,0,15,rfac);
         kgRoundedRectangleShade(fid,0.0,1.0,(float)xsize,(float)ysize,fillcolor,rfac,vmin*1.2,vmax);
        break;
        case 3:
         kgRoundedRectangleFill(fid,-1.0,1.0,(float)xsize,(float)ysize,0,15,rfac);
         kgRoundedRectangleShade0(fid,0.0,0.0,(float)xsize,(float)ysize,fillcolor,rfac,v,vmax*0.95);
        break;
        case 4:
         kgRoundedRectangleFill(fid,-1.0,+1.0,(float)xsize,(float)ysize,0,15,rfac);
         kgRoundedRectangleShade(fid,0.0,0.0,(float)xsize,(float)ysize,fillcolor,rfac,vmin,vmax*0.95);
        break;
        default:
         kgRoundedRectangleFill(fid,-1.0,0.5,(float)xsize,(float)ysize,0,15,rfac);
         kgRoundedRectangleFill(fid,0.0,0.0,(float)xsize,(float)ysize,0,fillcolor,rfac);
        break;
           
      }
//TCB
#if 0
   kgRoundedRectangle(fid,0.0,.0,(float)xsize+1.0,(float)ysize+1.0,D->gc.v_dim,rfac,0.5);
#endif
 kgLineColor(fid,0);
...
static void _uiMakeSBarImages(DIHB *B) {
 ...
 kgChangeColor(fid,0,0,0,0);
 kgChangeColor(fid,15,255,255,255);
 kgRoundedRectanglePressed(fid,0.,0.,(float)(l-of),(float)(w-of),D->gc.fill_clr,0.5,3);
 kgRoundedRectangleFill(fid,0.,0.,(float)(l-of-4.5),(float)(w-of-4.5),0,15,0.5);
 pt->bimg = (void *) kgGetResizedImage(fid);
 kgCloseImage(fid);
 w = w;
 l = pt->Sldwd+of;
 ...
 switch(type) {
        ...
        break;
        case 2:
         kgRoundedRectangleFill(fid,-1.0,1.0,(float)xsize,(float)ysize,0,150,rfac);
         kgRoundedRectangleShade(fid,0.0,1.0,(float)xsize,(float)ysize,fillcolor,rfac,vmin*1.2,vmax);
        break;
        case 3:
         kgRoundedRectangleFill(fid,-1.0,1.0,(float)xsize,(float)ysize,0,150,rfac);
        ...
        break;
        default:
         kgRoundedRectangleFill(fid,-1.0,0.5,(float)xsize,(float)ysize,0,150,rfac);
         kgRoundedRectangleFill(fid,0.0,0.0,(float)xsize,(float)ysize,0,fillcolor,rfac);
        break;
...
// kgRoundedRectangleFill(fid,(float).5,-.5,(float)(pt->l+3),(float)(3),0,140,0.5);
 kgRoundedRectangleFill(fid,(float).0,.0,(float)(pt->l+3),(float)(3.0),0,fillcolor,0.5);
 pt->bimg = (void *) kgGetResizedImage(fid);
 kgCloseImage(fid);
 w = pt->Sw+of;
 l = pt->Sldwd+of;
 ...
 HSVtoRGB(&red,&green,&blue,h,s,vmax);
 kgChangeColor(fid,150,(int)red,(int)green,(int)blue);
 kgRoundedRectangleFill(fid,(float)-1.0,0.0,(float)(w-of),(float)(w-of),0,150,0.5);
 kgRoundedRectangleFill(fid,(float)0.5,-0.5,(float)(w-of),(float)(w-of),0,0,0.5);
 kgRoundedRectangleFill(fid,(float)0.0,0.0,(float)(w-of),(float)(w-of),0,fillcolor,0.5);
...
// kgRoundedRectangleShade1(fid,0.0,0.0,(float)xsize,(float)ysize,fillcolor,rfac,vmin,vmin);
 ...
 kgRoundedRectangleFill(fid,(float)0.0,-.0,(float)(w-of),(float)(w-of),0,150,0.5);
 kgRoundedRectangleFill(fid,(float)0.0,0.0,(float)(w-of-1),(float)(w-of-1),0,153,0.5);
 kgRoundedRectangleFill(fid,(float)0.0,0.0,(float)(w-of-3.0),(float)(w-of-3.0),0,152,0.5);
 kgRoundedRectangleFill(fid,(float)0.0,0.0,(float)(w-of-6),(float)(w-of-6),0,151,0.5);
#endif
 kgLineColor(fid,0);
...
// pt->fimg =  (void *)kgGetResizedImage(fid);
 ...
 kgCloseImage(fid);
#else
 pt->fimg = kgBorderedRectangle((int)(w-of),(int)(w-of),color,0.5);
#endif
 if(B->direction==1) {
   void *timg;
   timg = kgRotateImage(pt->fimg,90);
...
static void _uiMakeSBarImagesType0(DIHB *B) {
 ...
 fillcolor = 127;
 kgChangeColor(fid,fillcolor,r,g,b);
 kgRoundedRectangleFill(fid,0.,0.,(float)(l-of),(float)(w-of),0,fillcolor,0.2);
...
#if 0
 kgRoundedRectangleFill(fid,(float)-1.0,0.0,(float)(w-of),(float)(w-of),0,15,0.5);
 kgRoundedRectangleFill(fid,(float)0.5,-0.5,(float)(w-of),(float)(w-of),0,0,0.5);
 kgRoundedRectangleFill(fid,(float)0.0,0.0,(float)(w-of),(float)(w-of),0,fillcolor,0.5);
#endif
// kgChangeColor(fid,fillcolor,220,220,220);
 ...
 kgChangeColor(fid,150,(int)red,(int)green,(int)blue);
 vmin= vmax*0.75;
 kgRoundedRectangleFill(fid,(float)-0.5,0.5,(float)xsize,(float)ysize,0,150,rfac);
 kgRoundedRectangleFill(fid,(float)0.5,-0.5,(float)xsize,(float)ysize,0,0,rfac);
 kgRoundedRectangleShade0(fid,0.0,0.0,(float)xsize,(float)ysize,fillcolor,rfac,vmin,vmax*0.95);
 kgLineColor(fid,0);
 kgMove2f(fid,0.0,ysize*.5);
 kgDraw2f(fid,0.0,-ysize*.5);
 kgLineColor(fid,15);
 ...
 void _uiup_dir(DIALOG *D,int x,int y,int w,float rfac,int type)
  {
   ...
   uiBkup_clip_limits(wc);
   uiSet_full_scrn(wc);
   uiShadedString(D,"!f35!w32!xs", x-1, EVGAY-y-1, w+2,w+2,35,D->gc.fill_clr,0,D->gc.v_dim,D->gc.FontSize-1,0,rfac,1,type);
   uiRest_clip_limits(wc);
  }
 void _uidown_dir(DIALOG *D,int x,int y,int w,float rfac,int type)
  {
   ...
   uiBkup_clip_limits(wc);
   uiSet_full_scrn(wc);
   uiShadedString(D,"!f35!w32!xt", x-1, EVGAY-y-1, w+2,w+2,35,D->gc.fill_clr,0,D->gc.v_dim,D->gc.FontSize-1,0,rfac,1,type);
   uiRest_clip_limits(wc);
  }
...
//  uiMenustr(D,x1+4,y2+5,br->menu[br->df-1],D->gc.menu_char);
  uiString(D,menu,x1,br->y1+yoff,ln,br->width,D->gc.MenuFont,D->gc.menu_char,D->gc.FontSize,-1,-1);
...
//  uiMenustr(D,x1+4,y2+5,br->menu[br->df-1],D->gc.menu_char);
#if 1
  uiString(D,menu,x1+5,y1,ln,w->width,D->gc.MenuFont,
...
void uiMakeYImages(DIY *w) {
   ...
   if(menu != NULL) {
     ...
     w->imgs[i]=NULL;
     for(i=0;i<items;i++) {
       w->imgs[i]= (void *) uiMenuStringImage(D,menu[i]->name,
...
void uiMakeRImages(DIRA *w,int lng) {
   ...
   if(menu != NULL) {
     ...
     for(i=0;i<items;i++) {
       w->imgs[i]= (void *) uiMenuStringImage(D,menu[i]->name,
                 lng-2*BxSize,th,D->gc.Font,D->gc.menu_char,
                 D->gc.FontSize,-1);
     }
#else
     w->imgs= (void **) uiMenuNailImages(D,menu,
                 lng-2*BxSize,th,D->gc.MenuFont,D->gc.menu_char,
                 D->gc.FontSize,-1,8);
#endif
   }
   return;
...
//   _uiclean_gui_area(D,X1,Y1,X2,Y2);
   _uirect_fill_transparent(WC(D),X1,D->evgay-Y1,X2,D->evgay-Y2,D->gc.fill_clr,D->transparency);
 return -1;
}
...
int Make_menu(DIALOG *D,DIE *w,int x1,int y1)
 {
   ...
   if(n!= 0) {n++;lngth=8;}
   lngth += n*9;
   w->bwsr = make_dialog_menu_str(D,w,*(w->df),*(w->df),w->menu,w->size);
   bwsr = (BRW_STR *)w->bwsr;
   bwsr->D= (DIALOG*)D; 
   bwsr->x1 +=x1;
   bwsr->y1 +=y1;
...
//   bwsr->pos=0;
   ...
   uiRest_clip_limits(wc);
    ...
    uiBkup_clip_limits(wc);
    uiSet_full_scrn(wc);
    _dvrect_fill_transparent(wc,(bwsr->x1),(bwsr->y1),(bwsr->x2), (bwsr->y2),D->gc.fill_clr,D->transparency);
    uiRest_clip_limits(wc);
#endif
   if(y->Bimg != NULL) kgRestoreImage(D,y->Bimg,bwsr->x1,bwsr->y1,(bwsr->x2-bwsr->x1+1),(bwsr->y2-bwsr->y1+1));
  }
  return(ret);
 }
...
int _uiMake_V(DIV *y)
 {
  ...
  if(y->hide != 1) {
   ...
   uiRest_clip_limits(wc);
    ...
    uiBkup_clip_limits(wc);
    uiSet_full_scrn(wc);
    _dvrect_fill_transparent(wc,(y->x1+x1),(y->y1+y1),(y->x2+x1), (y->y2+y1),D->gc.fill_clr,D->transparency);
    uiRest_clip_limits(wc);
#else
   RESTOREWIDGETAREA(y);
#endif
...
void uiDrawProgBartype0(DIO *o) {
   ...
   if(fid != NULL ) {
      ...
      if(bclr < 0) {
        bclr=-bclr;
        kgChangeColor(fid,777,bclr/1000000,(bclr%1000000)/1000,bclr%1000);
        bclr=777;
      }
      if(color < 0) {
        color=-color;
        kgChangeColor(fid,778,color/1000000,(color%1000000)/1000,color%1000);
        color=778;
      }
      kgLineColor(fid,bclr);
      kgRoundedRectangleFill(fid,xp,yp,(float)width,(float)height,
                 1,bclr,0.25);
      kgLineColor(fid,D->gc.ProgFillColor);
      kgRoundedRectangleFill(fid,xp,yp,(float)width-2.0,(float)height-3.0,
                 1,D->gc.ProgFillColor,0.25);
#if 0
      kgRoundedRectangle(fid,xp,yp,(float)width,(float)height,
                 D->gc.ProgBodrColor,0.25,0.5);
#endif
      kgLineColor(fid,color);
      if(o->direction==0) {
       if(lp>0) kgRoundedRectangleFill(fid,lp*0.5+offset,yp,lp,w,1,color,0.25);
     }
     else {
       if(o->percent >0) kgRoundedRectangleFill(fid,xp,lp*0.5+offset,l,lp,1,color,0.25);
     }
      img=kgGetResizedImage(fid);
...
void uiDrawProgBartype2(DIO *o) {
   ...
   if(fid != NULL ) {
      ...
      kgRoundedRectangle(fid,xp,yp,(float)width,(float)height,
                 bclr,0.2,0.2);
#else
      kgRoundedRectangleFill(fid,xp,yp,(float)width,(float)height,1,bclr,0.2);
      kgRoundedRectangleFill(fid,xp,yp,(float)width-2,(float)height-2,1,D->gc.fill_clr,0.2);
#endif
      kgLineColor(fid,color);
      if(o->direction==0) {
       if(lp>0) kgRoundedRectangleFill(fid,lp*0.5+offset,yp,lp,w,1,color,0.0);
      }
      else {
       if(o->percent >0) kgRoundedRectangleFill(fid,xp,lp*0.5+offset,l,lp,1,color,0.0);
      }
//      img=kgGetResizedImage(fid);
//      img=kgGetSmoothImage(fid);
      img=kgGetSharpImage(fid);
...
void uiDrawProgBartype3(DIO *o) {
   ...
   if(fid != NULL ) {
      ...
      kgLineColor(fid,bclr);
      if(o->direction==0) {
       kgRoundedRectangleFill(fid,l*0.5+offset,yp,(float)l,(float)w,
                 0,D->gc.ProgFillColor,0.5);
       if(lp>0) kgRoundedRectangleFill(fid,lp*0.5+offset,yp,lp,w,0,color,0.5);
       kgRoundedRectangleFill(fid,lp+offset,yp,(float)w*3.0,(float)w*3.0,
                 1,color,0.5);
       kgRoundedRectangleFill(fid,lp+offset,yp,(float)w*2.0,(float)w*2.0,
                 1,color,0.5);
     }
     else {
       kgRoundedRectangleFill(fid,xp,w*0.5+offset,(float)l,(float)w,
                 color,D->gc.ProgFillColor,0.5);
       if(o->percent >0) kgRoundedRectangleFill(fid,xp,lp*0.5+offset,l,lp,color,color,0.5);
       kgRoundedRectangleFill(fid,xp,lp+offset,(float)l*3.0,(float)l*3.0,
                 1,color,0.5);
       kgRoundedRectangleFill(fid,xp,lp+offset,(float)l*2.0,(float)l*2.0,
                 1,color,0.5);
     }
      img=kgGetResizedImage(fid);
//      img=kgGetSmoothImage(fid);
...
//    _dvrect_fill_transparent(wc,(x1),(y1),(x2), (y2),D->gc.fill_clr,D->transparency);
   CHECKLIMITS(o);
   if(o->Bimg== NULL) o->Bimg=kgGetBackground(D,x1-1,y1-1,x2+1,y2+1 );
   if(o->hide!=1) {
     switch(o->type) {
      default:
      case 0:
      ...
      case 3:
       ...
       break;
      case 1:
       if(o->Bimg!= NULL) kgRestoreImage(D,o->Bimg,x1-1,y1-1,x2-x1+3,y2-y1+3 );
       l = x2-x1-2*offset;
       w = y2 -y1-2*offset;
...
//       _dvbordertype0(D, x1+offset-2,y1+offset-2,x2+offset+2,y2-offset+2,D->gc.ProgBodrColor);
       _dvrect_fill(wc, x1+offset,y1+offset,x2-offset, y2-offset,D->gc.ProgFillColor);
       if(o->direction==0) {
         if(lp>0)  _dvrect_fill(wc, x1+offset,y1+offset,x1+offset+lp, y1+offset+w,color);
       }
       else {
         if(o->percent >0) _dvrect_fill(wc, x1+offset,y1+offset+lp,
                          x1+offset+l, y2-offset,color);
       }
      break;
     }
   }
  ...
  if(ui_Leftarrow(key) ) {
    if(t->nx == 1) {
      ...
      if(t->elmt[curbox].cursor > 0) {
        ...
        if((t->elmt[curbox].cursor < t->elmt[curbox].startchar) ) 
               t->elmt[curbox].startchar = t->elmt[curbox].cursor;
       if(code=='t')_ui_drawtextcursor(t);
       else _ui_drawtablecursor(t);
       uiUpdateOn(t->D);
      }
...
//    kgWriteImage(p->img,"/root/DIP.jpg");
   ...
   l = (y2 - y1);
   if(color >= 0) {
      _uibox_fill(wc,x1,D->evgay-y1,x2-2,D->evgay-y2+1,(unsigned int) color);
   }
   else {
         gc = D->gc;
         color = gc.fill_clr;
...
//       kgImage(D,img2,x1,y1,w,l,0.0,1.0);
    ...
    switch(p->bordr) {
      ...
      case 3:
        offset=4;
        offset=0;
        _dvbordertype0(D, x1-offset,y1-offset,x2+offset, y2+offset,gc.bright);
        break;
      case 1:
        offset=2;
        offset=0;
        _dvbordertype2(D, x1-offset,y1-offset,x2+offset, y2+offset,0,0,0,0);
        break;
      case 6:
        offset=4;
        offset=0;
...
void _uiDrawInfo (DIALOG* D,int item) {
   ...
   I->item=item;;
   I->Bimg = NULL;
   I->twin =  _uiput_txtwin(D,I->x1+D->xo,(I->y1+D->yo),I->x2+D->xo,(I->y2+D->yo));
   return;
}
int _uiMake_I(DII *I) {
  TWIN *Twin;
...
//    printf("%d %d %d\n",lw,y2,y1);
    ...
    I->linewidth=lw;
    uiPressedBoxFill(D,x1,y1,(x2-x1),(y2-y1),D->gc.info_fill,D->gc.fill_clr,3,0.0);
    Twin=(TWIN *)(I->twin);
...
//         kgImage(D,img,x1-1,y1,x2-x1+5,y2-y1,0.0,1.0);
         ...
         break;
         default:
         _uirect_fill(WC(D),x1,D->evgay-y1,x2-1,D->evgay-y2+1,tx->gc.txt_fill);
         dvdraw_depre(D,x1,y1,x2,y2);
         break;
       }
  ch =  df[ln];
...
//TCB
  ...
  cy = (elmt[curbox].y2+elmt[curbox].y1)/2+6;
  cx = elmt[curbox].x1+6+(elmt[curbox].cursor-elmt[curbox].startchar)*(Gap+Wd);
  cy = D->evgay-cy;
...
//TCB
   cy = (elmt[curbox].y2+elmt[curbox].y1)/2+6;
   cx = elmt[curbox].x1+6+(elmt[curbox].cursor-
        elmt[curbox].startchar)*(Gap+Wd);
   cy = D->evgay-cy;
   if(i==curbox) {
     _uirect_fill(WC(D),cx+Gap+1,cy-Bt,cx+Gap+1+Wd,cy+Ht,tx->gc.cur_clr);
   }
...
#if 0
  _uirect_fill(WC(D),elmt[curbox].x1,EVGAY-elmt[curbox].y1,elmt[curbox].x2-1,EVGAY-elmt[curbox].y2+1,tx->gc.txt_fill);
  uidraw_depre(D,elmt[curbox].x1,EVGAY-elmt[curbox].y1,elmt[curbox].x2,EVGAY-elmt[curbox].y2);
#endif
  ch =  df[ln];
  df[ln] ='\0';
   cy = (elmt[curbox].y2+elmt[curbox].y1)/2+6;
...
//  ln = tx->ln[curbox];
  ...
  cx = elmt[curbox].x1+T->FontSize/2+(elmt[curbox].cursor-elmt[curbox].startchar)*(T->FontSize);
  cy = (elmt[curbox].y1+2);
...
//     prsize = (Size(elmt[k].pr));
       uiString(D,elmt[k].pr,x1,y1-2,prsize-D->gc.FontSize,box_width,D->gc.PromptFont,tx->gc.txt_pchar,D->gc.FontSize,1,D->gc.fill_clr);
       x1 += prsize;
...
//         kgImage(D,img,x1-1,y1,x2-x1+5,y2-y1,0.0,1.0);
         ...
         break;
         default:
         _uirect_fill(wc,x1,D->evgay-y1,x2-1,D->evgay-y2+1,tx->gc.txt_fill);
         dvdraw_depre(D,x1,y1,x2,y2);
         break;
       }
#if 0
       ...
       if(noecho!=1) {
        uiSetGuiFixFontSize(D,D->gc.InputFontSize);
       _ui_putstring(D,x1+6,cy,elmt[k].df+elmt[k].startchar,tx->gc.txt_char);
       }
       else {
         char *stars,*str;
         int ln,i;
...
//  uidraw_inset(D,X1,D->evgay-1-Y1,X2,D->evgay-1-Y2);
  ...
  else _dvrect_fill_transparent(wc,(x1),(y1),(x2), (y2),D->gc.fill_clr,D->transparency);
  uiRest_clip_limits(wc);
  return 1;
 }
...
#if 1
    CHECKLIMITS(T);
    if(T->Bimg == NULL) T->Bimg= kgGetBackground(D,(X1),(Y1),(X2), (Y2));
    else kgRestoreImage(D,T->Bimg,(X1),(Y1),(X2-X1+1), (Y2-Y1+1));
    for(j=0;j<nx;j++) {
     prsize=0;
     tx->col=j;
     for(i=0;i<ny;i++) {
...
//       uiString(D,elmt[k].pr,x1,y1,prsize-D->gc.FontSize,box_width,D->gc.PromptFont,tx->gc.txt_pchar,D->gc.FontSize,1,D->gc.fill_clr);
       uiString(D,elmt[k].pr,x1,y1,prsize-D->gc.FontSize,box_width,D->gc.PromptFont,tx->gc.txt_pchar,D->gc.FontSize,1,-1);
       x1 += prsize;
       x2 = x1 + ((tx->elmt[k].ln)*9)+13;
       if(x2 > x2max ) x2max=x2;
       (tx->elmt[k].x1) = x1;
...
//         kgImage(D,img,x1-1,y1,x2-x1+5,y2-y1,0.0,1.0);
       ...
       if(k==0) {
         cx = elmt[k].x1+6+(elmt[k].cursor-elmt[k].startchar)*(Gap+Wd);
         cy = (elmt[k].y2+elmt[k].y1)/2+6;
         cy = D->evgay-cy;
         _uirect_fill(wc,cx+Gap+1,cy-Bt,cx+Gap+1+Wd,cy+Ht,tx->gc.cur_clr);
       }
       if(noecho!=1) {
       uiSetGuiFixFontSize(D,D->gc.InputFontSize);
       _ui_putstring(D,x1+6,(y1+y2)/2+6,elmt[k].df+elmt[k].startchar,tx->gc.txt_char);
       }
       else {
         char *stars,*str;
         int ln,i;
...
// _dvrect_fill_transparent(WC(D),(X1),(Y1),(X2), (Y2),D->gc.fill_clr,D->transparency);
    if(T->Bimg != NULL) kgRestoreImage(D,T->Bimg,(X1),(Y1),(X2-X1+1), (Y2-Y1+1));
  }
  uiRest_clip_limits(wc);
  return 1;
 }
...
int _ui_updatetextbox (void *tmp){
  DIT *T;
  TX_STR *tx;
  int n,ygap=0,xgap=0,k=0,i,j,l,x1,x2,y1,y2,box_width=20,temp;
  int xx1,yy1,xx2,yy2,xlng=0,lng=0,gap,pointer,cy;
...
//      stripblnk(elmt->df);
      ...
      while((i>=0)&&((wrk[i]<' ')||(wrk[i]>127))){wrk[i]='\0';i--;}
...
void _uiDrawTextBox (DIALOG* D,int item) {
   ...
   T_ELMT *elmt;
   DIALOG *Dtmp;
   int n,ygap=0,xgap=0,k=0,i,j,x1,x2,y1,y2,box_width=20,temp;
...
//  dy =wd+ygap;
  ...
  for(j=0;j<ny;j++) {
    ...
    for(k=0;k<nx;k++) {
      ...
      xpm = butns[i].xpmn;
      butns[i].imgn=uiMakeStringImage(D,butns[i].title,xpm,
                    x1,y1,ln+mfp,wd+mfp, D->gc.ButtonFont,bkgr,
                    0,D->gc.but_char,D->gc.GuiFontSize,0,B->fac,0,B->type);
      if(butns[i].xpmp != NULL) xpm = butns[i].xpmp ;
      else {
            timg = kgGetImageCopy(NULL,butns[i].xpmn);
            xpm  = kgChangeBrightness(timg,0.5);
      }
      butns[i].imgp=uiMakeStringImage(D,butns[i].title,xpm,
                    x1,y1,ln+mfp,wd+mfp, D->gc.ButtonFont,bkgr,
                    0,D->gc.but_char,D->gc.GuiFontSize,0,B->fac,-1,B->type);
      ...
      butns[i].imgh=uiMakeStringImage(D,butns[i].title,xpm,
                    x1,y1,ln+mfp,wd+mfp, D->gc.ButtonFont,bkgr,
                    highli,D->gc.but_char,D->gc.GuiFontSize,0,B->fac,0,B->type);
      kgFreeImage(timg); timg=NULL;
      i++;
    }
  }
...
*/
  ...
  Bkimg = kgGetBackground(D,x1,y1,x2,y2);
      ...
      if(butns[i].xpmp != NULL) timg = kgGetImageCopy(D,butns[i].xpmp);
      else {
      ...
      kgFreeImage(Bcopy); Bcopy=NULL;
      kgFreeImage(timg); timg=NULL;
      if(butns[i].xpmh != NULL) timg = kgGetImageCopy(D,butns[i].xpmh);
      else {
            timg = kgGetImageCopy(NULL,xpm);
...
//      y1 =butns[i].y1+ygap/2+5+wd;
      ...
      else kgRestoreImage(D,butns[i].Bimg0,x1,y1,(x2-x1+1),(y2-y1+1));
...
void _uiDrawB (DIALOG* D,int item) {
   ...
   switch(b->code) {
     case 'N':
       ...
       if(n!=0) n++;
       lngth = n*9+Hsize+2;
       b->buts = make_but_str(D,b->x1+x1+lngth,b->y1+y1+Hsize+2,b->xgap,
             b->ygap,b->nx,b->ny,b->lngth,b->width,*(b->df),
             b->titles,b->xpm,b->bkgr,sw);
       buts= (BUTS *)(b->buts);
       buts->D = D;
       buts->Hsize = D->Hsize;
       buts->type = b->type;
...
int _ui_processslidepress(S_STR *pt,KBEVENT kbe) {
  ...
  if (_uiGetSItem(kbe,xx+w+3,yy+3,xx+l-w-3,yy+w-3)){ mvnt = kbe.x-xx -w -3+2- pt->df;}
  if( mvnt) {
    df = pt->df;
    pt->df +=mvnt;
    if(pt->df < 0 ) pt->df=0;
...
//TCB
       ...
       switch(d->code) {
        case 'd':
         if(d->Update != NULL) d->Update(_ui_getdslidevalue(pt),pt->id,pt->D);
         break;
        default:
         break;
       }
...
int _ui_processhslidepress(DIHB *B,KBEVENT kbe) {
  ...
  switch(B->direction) {
    default:
    case 0:
    if (_uiGetSItem(kbe,xx1,yy1,xx1+l1,yy1+w)) mvnt = kbe.x-xx- pt->df;
    df = pt->df;
    pt->df +=mvnt;
    break;
    case 1:
    ...
    xx1 = pt->x-w/2;
    yy=pt->y;
    if (_uiGetSItem(kbe,xx1,yy1,xx1+w,yy1+l1)) mvnt = (kbe.y-yy-B->lngth+ pt->df);
    df = pt->df;
    pt->df =pt->df-mvnt;
    break;
  }
...
//           if(d->Update != NULL) d->Update(_ui_getdslidevalue(pt),i,D);
           switch(d->code) {
            ...
            case 'f':
             f = (DIF *)d;
             if(f->Update != NULL) f->Update(_ui_getfslidevalue(pt),pt->id,pt->D);
             break;
            default:
             break;
           }
...
#else
            count=0;
            while(((evnt=kgCheckEvent(pt->D,&kb))!=0)&&(kb.event==3)) {
               count++;
               kbe= kb;
               if(count==50) break;
            }
  ...
  if (_uiGetSItem(kbe,x1,y1,x2,y2)) {
     do {
       ...
       if(mvnt) {
         ...
         if(df != pt->df) {
           _ui_movehslidepointer(B);
           if(B->Update != NULL) B->Update(_ui_getdslidevalue(pt),B->item,D);
...
//   int x2,y2,l;
   ...
   uiString(D,m,x1,y1,(x2-x1),(y2-y1),D->gc.MsgFont,D->gc.msg_char,D->gc.FontSize,0,D->gc.msg_fill);
   _ui_draw_bound((D),x1,EVGAY-(y1),x2,EVGAY-y2,D->gc.msg_bodr);
   uiRest_clip_limits(wc);
  }
  void _ui_dia_T_message(DIALOG *D,int x1,int y1,char *m)
  {
   ...
   x2 = x1+ l*9+10;
   y2 = y1+16;
   uiString(D,m,x1,y1,(x2-x1),abs(y2-y1),D->gc.MsgFont,D->gc.msg_char,D->gc.FontSize,0,D->gc.msg_fill);
   uiRest_clip_limits(wc);
  }
 ...
 if(m->hide != 1) {
   ...
   switch(m->code) {
       case 'm':
         ...
         uiString(D,m->msg,(int)(x1),(int)((y1)),(x2-x1),(y2-y1),D->gc.MsgFont,D->gc.msg_char,D->gc.FontSize,just,fillclr);
         break;
       case 'M':
         FontSize = 5*h/9;
         uiSplashString(D,m->msg,(int)(m->x1+D->xo),(int)((m->y1+D->yo)),w,h,D->gc.SplashFont,D->gc.SplashFillColor,D->gc.SplashCharColor,FontSize,just,0.25);
         break;
       case 'B':
//           _ui_diamessage(D,m->x1+D->xo,m->y1+D->yo,m->x2+D->xo,m->y2+D->yo,m->msg);
           ...
           uiString(D,m->msg,x1,y1,w,h,D->gc.MsgFont,D->gc.msg_char,D->gc.FontSize,just,D->gc.msg_fill);
    ...
    y1 = m->y1+D->yo;
    y2 = m->y2+D->yo;
    if(m->Bimg != NULL) kgRestoreImage(D,m->Bimg,x1,y1,(x2-x1+1),(y2-y1+1));
  }
  return ret;
}
        ...
        if(item){
           do {
            ...
            if(br->MS.nitems-br->size >0){
              ...
              if(br->hitem > br->MS.nitems ) br->hitem = br->MS.nitems;
              br->df = br->hitem;
              if(br->df != df ) {_uiMoveVertPointer(br);_uiPutmenu(br);uiUpdateOn(br->D);}
              ret =-1;
            }
#if 0
            count=0;
...
#else
            count=0;
            while(((evnt=kgCheckEvent(br->D,&kb))!=0)&&(kb.event==3)) {
               count++;
               kbe= kb;
               if(count==50) break;
            }
...
int kgGetThumbNailItem(DIY *Y,int x,int y) {
      ...
      yoffset = br->offset;
      xoffset = Y->offset;
      pos=_uiGetSItem(kbe,br->x1+xoffset,br->y1+yoffset,br->x2-xoffset-br->w,br->y2-yoffset);
      if ( pos) {
...
int _ui_process_y_click(DIY *y,KBEVENT kbe) {
      ...
      wc = br->D->wc;
      df = br->df;
      item=_uiGetSItem(kbe,br->rsx1,br->rsy1+br->sy,br->rsx2,br->rsy1+br->sy+br->shy);
      if(item){
      ...
      uiBkup_clip_limits(wc);
      uiSet_full_scrn(wc);
      item=_uiGetSItem(kbe,br->x1+xoffset,br->y1+yoffset,br->x2-xoffset-br->w,br->y2-yoffset);
      if ( item) {
...
#else
            count=0;
            while(((evnt=kgCheckEvent(y->D,&kb))!=0)&&(kb.event==3)) {
               count++;
               kbe= kb;
               if(count==50) break;
            }
...
//        printf("_ui_process_r_move\n");
      ...
      uiBkup_clip_limits(wc);
      uiSet_full_scrn(wc);
      item=_uiGetSItem(kbe,br->x1+xoffset,br->y1+offset,br->x2-xoffset-br->w,br->y2-offset);
      if ( item) {
       if(kbe.button != 1){ uiRest_clip_limits(wc); return -1; }
      ...
      if ( item) {
        ...
        if(item){
            ...
            if(br->MS.nitems-br->size >0){
              ...
              if(br->hitem < 1) br->hitem=1;
              if(br->hitem > br->MS.nitems ) br->hitem = br->MS.nitems;
              if(br->hitem != df ) {_uiMoveVertPointer(br);_uiPutmsg(br);uiUpdateOn(br->D);}
              ret =-1;
            }
        }
      }
...
void _uiPutmenu(BRW_STR *br) {
      ...
      ixmid=br->MS.ixx+br->MS.ixl/2;
      iy = br->MS.iyy+1;
      _uirect_fill(WC(D),(br->MS.ixx+1),(EVGAY-br->MS.iyy),(br->MS.ixu-1),
                (EVGAY-br->MS.iyu),br->MS.color1);
#if 0
#else
#if 1
    if(br->D!=NULL) _ui_draw_bound(D,br->MS.ixx+1,(EVGAY-br->MS.iyy-1),br->MS.ixu-1,(EVGAY-br->MS.iyu+1),br->MS.color2);
    else _ui_draw_menu_bound(D,br->MS.ixx,(EVGAY-br->MS.iyy),br->MS.ixu,(EVGAY-br->MS.iyu),br->MS.color2);
#else
    _ui_draw_menu_bound(D,br->MS.ixx,(EVGAY-br->MS.iyy),br->MS.ixu,(EVGAY-br->MS.iyu),br->MS.color2);
#endif
#endif
      k =1;
      if((br->MS.item>0) && (br->MS.item<=br->MS.imenu)) k = br->MS.item;
      jj = iy + (k)*br->MS.thirty;
      _uirect_fill(WC(D),(br->MS.ixx+4),(EVGAY-jj-12),(br->MS.ixu-4),
                (EVGAY-jj+8),br->MS.color2);
      jj = iy;
//TCB
      uiDefaultGuiFontSize(D);
...
#if 0
        iyp = EVGAY-jj-7;
        uiwrite_string(D,br->MS.menu[kk],(int)ixp,(int)iyp,br->MS.char_clr);
#else
          ixp = br->MS.ixx+5;
          iyp = jj+12-br->MS.thirty;
          uiString(D,br->MS.menu[kk],(int)ixp,(int)iyp,ln,br->MS.thirty,D->gc.MsgFont,D->gc.msg_char,D->gc.FontSize,-1,-1);
#endif
      }
      uiRest_clip_limits(WC(D));
  }
...
//      int EVGAY,EVGAX;
      ...
      list = (ThumbNail **) y->list;
      k=0; while(list[k]!=NULL) {if(list[k]->sw != 0) list[k]->sw=1;k++;}
      scroll=br->scroll;
      w = y->w*scroll;
      th = y->ygap-6;
      yoffset=br->offset;
...
void _uiPutCheckBox( DICH *y){
      ...
      int n,iy,iyp,ixp,ixmid;
      int scroll=1,*sw,swv,nx,ny,yi,xi,scrsize,xoffset,yoffset,th,w,xgap,lngth,bxln;
      int EVGAY;
      DIALOG *D;
      void *xpm=NULL;
      GMIMG *img;
...
//      int EVGAY,EVGAX;
      ...
      if(y->nitems > 1) {
       ...
       if(y->itemhi) {
        ...
        yi = br->hitem-pos;
        iyp = br->y1+yoffset+yi*br->width;
        _dvrect_fill(WC(D),ixp+2,iyp+2,br->x2-xoffset-w-2,iyp+br->width-2,D->gc.high_clr);
       }
#else
       if(y->itemhi) {
        xi = (br->hitem%y->nx);
        ...
        iyp = br->y1+yoffset+yi*br->width;
        _dvrect_fill(WC(D),ixp+2,iyp+2,ixp+bxln-2,iyp+br->width-2,D->gc.ItemHighColor);
       }
#endif
      }
      kk=0;
        ...
        for(xi=0;xi<nx;xi++) 
      {
        ...
        if(xpm!= NULL){
         ...
         kgImage(D,xpm,ixp+2,iyp+4,y->width+y->xgap-4,y->width,0.0,1.0);
        }
#endif
#if 0
	if((menu!= NULL)) uiCheckString(D,menu[kk]->name,
                      (int)ixp,(int)iyp,bxln,th,D->gc.Font,
                      D->gc.menu_char,D->gc.FontSize,swv);
#endif
        if(menu != NULL) {
           kgImage(D,y->imgs[kk+pos],(int)(ixp+3.0*D->gc.FontSize),(int)iyp,
                   (bxln -4*D->gc.FontSize),th,0.0,1.0);
           if(swv==0) kgImage(D,y->nimg,(int)ixp,(int)iyp,4*D->gc.FontSize,th,0.0,1.0);
           if(swv==1) kgImage(D,y->himg,(int)ixp,(int)iyp,4*D->gc.FontSize,th,0.0,1.0);
        }
        kk++;
      }
      uiRest_clip_limits(WC(D));
...
#if 0
          if(y->type<=1 ) 
              uiMenuString(D,menu[kk]->name, (int)ixp,(int)iyp,bxln,y->width,D->gc.Font,
                    D->gc.menu_char,D->gc.FontSize,swv);
          else 
              uiString(D,menu[kk]->name, (int)ixp,(int)iyp,bxln,y->width,D->gc.Font,
                    D->gc.menu_char,D->gc.FontSize,-1,-1);
#else
          if(((y->type)%10)<=1 )  {
            kgImage(D,y->imgs[kk+pos],(int)ixp,(int)iyp,bxln-y->width,y->width,0.0,1.0);
            if(swv==0) kgImage(D,y->nimg,(int)ixp+bxln-y->width,(int)iyp,
                               y->width,y->width,0.0,1.0);
            if(swv==1) kgImage(D,y->himg,(int)ixp+bxln-y->width,(int)iyp,
                               y->width,y->width,0.0,1.0);
          }
          else {
             kgImage(D,y->imgs[kk+pos],(int)ixp,(int)iyp,bxln,y->width,0.0,1.0);
          }
#endif
        }
        kk++;
...
void _uiPutEmenu( DIE *y){
      ...
      if(y->nitems > 1) {
       if(y->itemhi) {
        ...
        yi = br->hitem-pos;
        iyp = br->y1+yoffset+yi*br->width;
        _dvrect_fill(WC(D),ixp+2,iyp+2,br->x2-xoffset-w-2,iyp+br->width-2,D->gc.ItemHighColor);
       }
      }
#endif
      kk=0;
...
//                    D->gc.menu_char,D->gc.FontSize,-1,-1);
            kgImage(D,y->imgs[kk+pos], (int)ixp+xoff*y->width,(int)iyp,bxln,y->width,0.0,1.0);
        }
        kk++;
      }
      uiRest_clip_limits(WC(D));
...
void _uiPutRadioButton( DIRA *y){
      ...
      int k,kk,knew,jj,ln,ln1;
      int n,iy,iyp,ixp,ixmid;
      int scroll=1,swv,nx,ny,yi,xi,scrsize,xoffset,yoffset,th,w,xgap,lngth,bxln;
      int EVGAY;
      DIALOG *D;
      void *xpm=NULL;
      GMIMG *img;
...
#if 0
	if((list!= NULL)&&(menu[kk]->name!=NULL)) uiRadioString(D,menu[kk]->name,
                      (int)ixp,(int)iyp,bxln,th,D->gc.Font,
                      D->gc.menu_char,D->gc.FontSize,swv);
#endif
        kk++;
...
//      _uirect_fill(WC(D),(br->MS.ixx-1),(EVGAY-br->MS.iyy),(br->MS.ixu+1),(EVGAY-br->MS.iyu),br->gc.info_fill);
      _uirect_fill(WC(D),(br->MS.ixx),(EVGAY-br->MS.iyy-1),(br->MS.ixu),(EVGAY-br->MS.iyu+1),br->gc.msg_fill);
      _ui_draw_bound(D,br->MS.ixx,(EVGAY-br->MS.iyy-1),br->MS.ixu,(EVGAY-br->MS.iyu+1),br->gc.msg_bodr);
      k =1;
      jj = iy + (k)*br->MS.thirty;
      jj = iy;
      uiSetGuiFixFontSize(D,D->gc.InputFontSize);
...
//        write_string(br->MS.menu[kk],(int)ixp,(int)iyp,br->gc.info_char);
        if((br->menu != NULL) &&((kk+pos) <n)&&(br->menu[kk+pos]!= NULL)) {
           uiwrite_string(D,br->menu[kk+pos],(int)ixp,(int)iyp,br->gc.msg_char);
        }
        else uiwrite_string(D," ",(int)ixp,(int)iyp,br->gc.msg_char);
      }
      uiDefaultGuiFontSize(D);
      uiRest_clip_limits(WC(D));
  }
...
//   scr_ln=h-2*w-8;
   ...
   if(sh<MINV) sh = MINV;
   fac =0.0;
   if(br->MS.nitems-br->size !=0)fac=(scr_ln-sh)/(float)(br->MS.nitems-br->size);
   else return;
   br->MS.fac =fac;
//   pos=br->df-br->size;
   if(br->pos < (br->hitem-br->size) )br->pos = br->hitem-br->size;
...
void _uiMoveYVertPointer(void *tmp) {
  ...
  D = (DIALOG *)(br->D);
   ...
   if(sh<MINV) sh = MINV;
   fac =0.0;
   if((y->ny-y->size) >0)fac=(scr_ln-sh)/(float)(y->ny-y->size);
   else return;
   br->MS.fac =fac;
   br->fac = fac;
   pos = (br->pos)/(nx) ;
...
void _uiMoveEVertPointer(void *tmp) {
  ...
  D = (DIALOG *)(br->D);
   ...
   if(sh<MINV) sh = MINV;
   fac =0.0;
   if((y->nitems-y->size) >0)fac=(scr_ln-sh)/(float)(y->nitems-y->size);
   else return;
   br->MS.fac =fac;
   br->fac = fac;
   pos = (br->pos)/(nx) ;
...
void _uiDrawScrollMenu(BRW_STR *br) {
  ...
  kg_scr_back(WC(D), br->MS.ixx-8,br->MS.iyy-18,(br->MS.ixx+br->MS.ixl+31),(br->MS.iyy+br->MS.iyl+18));
  uiBkup_clip_limits(WC(D));
  uiSet_full_scrn(WC(D));
   _uirect_fill(WC(D), br->MS.ixx-8,EVGAY-br->MS.iyy+18,(br->MS.ixx+br->MS.ixl+31),
                 EVGAY-(br->MS.iyy+br->MS.iyl+18),gc.fill_clr);
//   _uiborder3d(D, br->MS.ixx-8,br->MS.iyy-18,(br->MS.ixx+br->MS.ixl+31),(br->MS.iyy+br->MS.iyl+18));
   _uibordertype4(D, br->MS.ixx-8,br->MS.iyy-18,(br->MS.ixx+br->MS.ixl+31),(br->MS.iyy+br->MS.iyl+18));
   uiRest_clip_limits(WC(D));
   pos  = 1;
   xx=br->MS.ixx+br->MS.ixl+1;
   yy = br->MS.iyy-10;
...
#else
   if(y->Bimg!=NULL) kgRestoreImagePart(D,y->Bimg,x1+1,y1+1,1,1,(x2-x1-1),(y2-y1-1));
#endif
...
//   _dvrect_fill(WC(D), x1,y1,x2,y2,gc.fill_clr);
   if(y->Bimg!=NULL) kgRestoreImage(D,y->Bimg,x1,y1,(x2-x1+1),(y2-y1+1));
  }
  uiRest_clip_limits(WC(D));
//  uiUpdateOn(D);
  return;
...
//     uiwrite_string(D,ch1,menu_x,menu_y,char_clr);
     uiString(D,ch1,ix,iy,max1*9,22,D->gc.MsgFont,char_clr,D->gc.FontSize,-1,fill_clr);
#endif
  uiDefaultGuiFontSize(D);
  uiset_clr(D,c_color);
}
void uiscroll_up_twin(DIALOG *D,TWIN *Twin) {
  ...
  uig_scroll_up(D,(int)Twin->xl,EVGAY-(int)Twin->yl,(int)Twin->xu,
              EVGAY-(int)Twin->yu,Twin->w);
#else
    int yy1,yy2,jmp;
    yy1 = (int)Twin->yl;
    yy2 = (int)Twin->yu;
    jmp = Twin->w;
    if(yy1+jmp > yy2 ) return;
    uiscr_scroll_back(D,(int)Twin->xl,yy1+jmp,(int)Twin->xu,yy2,jmp);
#endif
  return;
}
...
void ui_txtwin(DIALOG *D,int ix,int iy,int chrs,int lines)
 {
  ...
  _uirect_fill(WC(D),(int)ix,(int)EVGAY-iy,(int)x2,(int)EVGAY-y2,D->gc.twin_fill);
...
int _uiMenu(DIW *w) {
  ...
  int n=0,i,j,k=0,item,ok,prop,max,it,scr_ln=0,jold,Attn=0;
  float fac=1.;
  DIALOG *D;
  KBEVENT kbevent;
  br = w->bwsr;
